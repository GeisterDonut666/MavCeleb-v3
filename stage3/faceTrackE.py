...
def crop_video(inputVideo,track,cropfile,tmpDir):
    ...
    cap.release()
    vOut.release()
    command = ["ffmpeg -y -i %s -ac 1 -vn -acodec pcm_s16le -ar 16000 -ss %.3f -to %.3f %s" % (os.path.join(tmpDir,'video.avi'),audiostart,audioend,audiotmp)]
...
#for ty in glob.glob(ids+'\\English'):
    ...
    #for links in glob.glob(ty+'\\*\\*.*'):
        ...
        try:
            print(all_links.index(links))
        except ValueError:
            pass
        # Rename files so they don't include withespaces
        new_name = links.replace(" ", "_")
        os.rename(links, new_name)
        links = new_name
        print(links)
        ...
        if not os.path.exists(tp):
            os.makedirs(tp)
        try:
            #subprocess.call([f'ffmpeg -y -i {tmpDir}video.mp4 {tmpDir}video.avi'], shell=True, stdout=None)
            #subprocess.call(['ffmpeg', '-y', '-i', f'{tmpDir}video.mp4', f'{tmpDir}video.avi'], shell=True, stdout=None)
            subprocess.call([f'ffmpeg -y -i {links} -qscale:v 4 -r 25 {tmpDir}video.avi'], shell=True, stdout=None)
            ...
            print('error in:',tp)
            with open('viderrE.txt','a+') as file:              # if video not processed, writes into log and continues
                file.write(tp+'\n')
